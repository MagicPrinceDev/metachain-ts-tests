...
describeWithMetachain('Metachain RPC (Transaction cost)', (context) => {
	step('should take transaction cost into account and not submit it to the pool', async function () {
		...
		let msg = `Custom error: Could not publish raw transaction: ${rawtx} reason: Test EvmTxTx execution failed:\nevm tx failed to validate tx gas price is lower than initial block base fee`;
