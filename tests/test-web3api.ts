...
describeWithMetachain('Metachain RPC (Web3Api)', (context) => {
	// TODO(canonbrother): web3_clientVersion uncomment once ready
	// step('should get client version', async function () {
	// 	const version = await context.web3.eth.getNodeInfo();
	// 	expect(version).to.be.contains(`${RUNTIME_SPEC_NAME}/v${RUNTIME_SPEC_VERSION}`);
	// });
	// TODO(canonbrother): uncomment once ready
